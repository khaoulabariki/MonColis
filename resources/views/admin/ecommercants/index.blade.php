...
<div class="container-fluid my-6">
    ...
    <div id="addEcomForm" class="hidden bg-white rounded-3xl border border-slate-200/60 shadow-xl shadow-slate-100/50 p-8 mb-8 transition-all">
        <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
            <div>
                <h3 class="text-xl font-black text-slate-900 tracking-tight">
                    <i class="fas fa-store text-[#0A4BB3] me-2"></i>{{ __('Nouveau E-commerçant') }}
                </h3>
                <p class="text-xs font-medium text-slate-400 mt-0.5">{{ __('Créez un nouveau profil e-commerçant sur la plateforme.') }}</p>
            </div>
        ...
        <form action="{{ route('admin.ecommercants.store') }}" method="POST" class="space-y-6 m-0 p-0">
            @csrf
    ...
    <div class="bg-white rounded-3xl border border-slate-200/60 shadow-xs overflow-hidden">
        ...
        <div class="p-6 sm:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">{{ __('Partenaires & Boutiques') }}</span>
                <h2 class="text-2xl font-black text-slate-900 tracking-tight">{{ __('Liste des E-commerçants') }}</h2>
                <p class="text-slate-400 text-xs mt-0.5 font-medium">{{ __('Gérez les comptes des e-commerçants inscrits sur la plateforme.') }}</p>
            </div>
            
            <button onclick="toggleForm()" class="bg-[#0A4BB3] hover:bg-[#083da3] text-white font-black text-xs uppercase tracking-widest px-5 py-3 rounded-2xl transition shadow-lg shadow-blue-900/10 flex items-center gap-2.5 cursor-pointer border-0">
                <i class="fas fa-plus text-[11px]"></i> {{ __('Ajouter un E-commerçant') }}
            </button>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 font-medium">
                    @forelse($ecommercantsList as $ecom)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="py-5 px-6 font-mono text-xs text-slate-500">{{ $ecom->email }}</td>
                            <td class="py-5 px-6 text-slate-500">{{ $ecom->telephone }}</td>
                            <td class="py-5 px-6 text-center">
                                <div class="flex justify-center items-center">
                                    <form action="{{ route('admin.ecommercants.destroy', $ecom->id) }}" method="POST" onsubmit="return confirm(@json(__('Êtes-vous sûr de vouloir supprimer ce e-commerçant ?')));" class="inline m-0 p-0">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-10 text-center text-slate-400 font-bold">
                                <i class="fas fa-store-slash text-2xl block mb-2 text-slate-300"></i> {{ __('Aucun e-commerçant trouvé dans la base de données.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
