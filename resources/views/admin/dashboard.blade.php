...
@section('content')
<div class="mb-10">
    <span class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-1.5">{{ __('Tableau de Bord') }} · {{ __('Admin') }}</span>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 mb-10">
    ...
    <div class="bg-[#0A4BB3] text-white p-7 rounded-3xl shadow-lg shadow-blue-900/10 flex flex-col justify-between min-h-[210px]">
        <div>
            ...
            <span class="text-[11px] font-black tracking-wider uppercase text-blue-200">{{ __('Colis Total ce mois') }}</span>
        </div>
    ...
    <div class="bg-[#10B981] text-white p-7 rounded-3xl shadow-lg shadow-emerald-950/10 flex flex-col justify-between min-h-[210px]">
        <div>
            ...
            <span class="text-[11px] font-black tracking-wider uppercase text-emerald-100">{{ __('Livrés avec succès') }}</span>
        </div>
        <div class="flex items-end gap-2 h-8 mt-5 opacity-40">
            <div class="bg-white w-full h-4 rounded-xs"></div>
            <div class="bg-white w-full h-6 rounded-xs"></div>
    ...
    <div class="bg-[#FF6B00] text-white p-7 rounded-3xl shadow-lg shadow-orange-950/10 flex flex-col justify-between min-h-[210px]">
        <div>
            <div class="flex justify-between items-center mb-5">
                ...
                <span class="text-[10px] font-bold bg-white/20 px-2.5 py-1 rounded-full text-white/90 tracking-wide">{{ __('En cours') }}</span>
            </div>
            <span class="text-4xl font-black tracking-tight block mb-1.5">{{ $enCours ?? 0 }}</span>
            <span class="text-[11px] font-black tracking-wider uppercase text-orange-100">{{ __('En cours de livraison') }}</span>
        </div>
        <div class="flex items-end gap-2 h-8 mt-5 opacity-40">
            <div class="bg-white w-full h-6 rounded-xs"></div>
            <div class="bg-white w-full h-7 rounded-xs"></div>
    ...
    <div class="bg-[#EF4444] text-white p-7 rounded-3xl shadow-lg shadow-rose-950/10 flex flex-col justify-between min-h-[210px]">
        <div>
            ...
            <span class="text-[11px] font-black tracking-wider uppercase text-rose-100">{{ __('Retournés / Refusés') }}</span>
        </div>
        <div class="flex items-end gap-2 h-8 mt-5 opacity-40">
            <div class="bg-white w-full h-5 rounded-xs"></div>
            <div class="bg-white w-full h-4 rounded-xs"></div>
    ...
    <div class="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-7 rounded-3xl shadow-lg shadow-indigo-950/20 flex flex-col justify-between min-h-[210px]">
        <div>
            <div class="flex justify-between items-center mb-5">
                ...
                <span class="text-[10px] font-bold bg-white/20 px-2.5 py-1 rounded-full text-emerald-400 tracking-wide">{{ __('Net') }}</span>
            </div>
            ...
            <span class="text-[10px] font-black tracking-wider uppercase text-indigo-200">{{ __('Revenu Net Agence') }}</span>
        </div>
        <div class="flex items-end gap-2 h-8 mt-5 opacity-40">
            <div class="bg-white w-full h-6 rounded-xs"></div>
            <div class="bg-white w-full h-8 rounded-xs"></div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">{{ __('Ventilation Graphique des Colis') }}</h3>
    ...
    <div class="bg-white rounded-3xl p-8 border border-slate-200/60 shadow-xs flex flex-col justify-between min-h-[420px]">
        <div>
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="text-[10px] font-black bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full tracking-wide">{{ __('Analyse IA Live') }}</span>
            </div>
            
            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">{{ __('Satisfaction & Sentiments Clients') }}</h3>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-8">
                <div class="w-36 h-36 relative flex items-center justify-center shrink-0">
                    ...
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span class="text-[9px] text-slate-400 font-bold uppercase tracking-wider">{{ __('Score Global') }}</span>
                    </div>
                </div>
                <div class="space-y-2.5 w-full">
                    <div class="flex items-center justify-between text-xs font-bold text-slate-600 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100"><span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-[#10B981]"></span> {{ __('Positif') }}</span> <span class="font-mono text-emerald-600 font-black">{{ $pctPositif }}%</span></div>
                    <div class="flex items-center justify-between text-xs font-bold text-slate-600 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100"><span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-[#0A4BB3]"></span> {{ __('Neutre') }}</span> <span class="font-mono text-blue-600 font-black">{{ $pctNeutre }}%</span></div>
                    <div class="flex items-center justify-between text-xs font-bold text-slate-600 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100 text-rose-600"><span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-[#EF4444]"></span> {{ __('Négatif') }}</span> <span class="font-mono text-rose-600 font-black">{{ $pctNegatif }}%</span></div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 pt-5 border-t border-slate-100 flex flex-col gap-2.5">
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <i class="fas fa-brain text-purple-500"></i> {{ __("Résumé Analytique de l'Intelligence Artificielle :") }}
            </span>
            <div class="bg-gradient-to-r from-slate-50 to-blue-50/30 p-4 rounded-xl border border-blue-100 text-xs text-slate-700 font-semibold leading-relaxed relative overflow-hidden">
                <div class="absolute end-2 bottom-2 text-slate-200/40 text-4xl"><i class="fas fa-quote-right"></i></div>
                @if($totalAvis > 0)
                    "{!! __("D'après l'analyse sémantique de l'IA sur les :total retours récents, la plateforme maintient un excellent niveau de performance globale avec :pct d'avis positifs. Les points forts résident dans la rapidité de traitement des clôtures. Quelques flux logistiques mineurs demandent une attention préventive.", ['total' => $totalAvis, 'pct' => "<span class='text-emerald-600 font-black'>{$pctPositif}%</span>"]) !!}"
                @else
                    "{{ $iaResume ?? __("L'analyse prédictive globale de l'IA indique un niveau de satisfaction client optimal stabilisé à :pct%. Aucun signal critique ou anomalie de livraison n'a été détecté dans les derniers flux d'avis enregistrés.", ['pct' => $pctPositif]) }}"
                @endif
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        new Chart(ctxColis, {
            type: 'doughnut',
            data: {
                labels: [@json(__('Livrés')), @json(__('En cours')), @json(__('Retournés')), @json(__('Annulés'))],
                datasets: [{
...
<div class="mt-8 bg-white rounded-3xl shadow-xs border border-slate-200/60 overflow-hidden w-full p-6 text-start">
    <div class="mb-6">
        <h3 class="text-sm font-black text-slate-950 uppercase tracking-wider flex items-center gap-2">
            <i class="fas fa-comments text-brand-blue"></i> {{ __('Tous les Avis Clients') }}
        </h3>
        <p class="text-[11px] font-medium text-slate-400 mt-0.5">{{ __("Liste globale des retours d'expérience sur la plateforme.") }}</p>
    </div>
        ...
        <table class="w-full text-start border-collapse text-sm text-slate-600">
            <thead class="bg-slate-50/70 text-slate-400 font-black uppercase text-[10px] tracking-widest border-b border-slate-100">
                <tr class="text-start">
                    ...
                    <th class="p-4 text-start">{{ __('Commentaire / Avis') }}</th>
            ...
            <tbody class="divide-y divide-slate-100 font-bold text-xs">
                @forelse($recentAvis as $avis)
                    <tr class="hover:bg-slate-50/30 transition">
                        ...
                        <td class="p-4 text-slate-900 font-black">{{ $avis->colis->prenom_destinataire ?? '' }} {{ $avis->colis->nom_destinataire ?? __('Client Anonyme') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="p-12 text-center text-slate-400 font-medium">{{ __('Aucun commentaire enregistré.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
