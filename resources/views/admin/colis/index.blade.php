...
@section('content')
        ...
        <div class="max-w-3xl mx-auto bg-white rounded-3xl border border-slate-200/60 shadow-xl shadow-slate-100/50 p-8 my-6">
            <div class="mb-8">
                <a href="{{ route('admin.colis.index') }}" class="text-[#0A4BB3] text-xs font-black tracking-wider uppercase hover:underline flex items-center gap-1">
                    <i class="fas fa-arrow-left text-[10px] rtl:rotate-180"></i> {{ __('Retour à la liste') }}
                </a>

                {{-- Titre dynamique selon le contexte --}}
                <h1 class="text-2xl font-black text-slate-900 tracking-tight mt-3">
                    {{ isset($colisEnCours) && $colisEnCours ? __('Modifier le Colis') : __('Ajouter un Nouveau Colis') }}
                </h1>
                <p class="text-xs font-medium text-slate-400 mt-0.5">{{ __('Renseignez les informations requises pour le traitement du colis.') }}</p>
            </div>
            ...
            <form action="{{ route('admin.colis.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Code Suivi') }}</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('E-commerçant (Propriétaire)') }}</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Nom du Destinataire') }}</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Prénom du Destinataire') }}</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Téléphone Destinataire') }}</label>
                        <input type="text" name="telephone"
                               value="{{ isset($colisEnCours) && $colisEnCours ? $colisEnCours->telephone_destinataire : '' }}"
                               required class="w-full px-4 py-3 border border-slate-200 focus:border-[#0A4BB3] rounded-xl text-sm font-medium text-slate-800 focus:outline-none transition">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Ville / Adresse de Destination') }}</label>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{{ __('Prix (DH)') }}</label>
                ...
                <div class="pt-6 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('admin.colis.index') }}" class="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-black uppercase tracking-wider rounded-xl transition">{{ __('Annuler') }}</a>
                    <button type="submit" class="px-6 py-2.5 bg-[#0A4BB3] hover:bg-[#083da3] text-white text-xs font-black uppercase tracking-wider rounded-xl transition shadow-lg shadow-blue-900/10">
                        {{ isset($colisEnCours) && $colisEnCours ? __('Mettre à jour') : __('Enregistrer le colis') }}
                    </button>
                </div>
            </form>
        </div>
    ...
    @else
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">{{ __('Gestion des expéditions') }}</span>
            ...
            <a href="{{ route('admin.colis.create') }}" class="bg-[#0A4BB3] hover:bg-[#083da3] text-white text-xs font-black uppercase tracking-widest px-5 py-3 rounded-2xl transition shadow-lg shadow-blue-900/10 flex items-center gap-2.5 decoration-none">
                <i class="fas fa-plus text-[10px]"></i> {{ __('Ajouter un Colis') }}
            </a>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-start border-collapse text-sm text-slate-600">
                    <thead class="bg-slate-50/70 text-slate-400 font-black uppercase text-[10px] tracking-widest border-b border-slate-100">
                        <tr>
                            <th class="py-5 px-6 text-start">{{ __('Code Suivi') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Destinataire') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Téléphone') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Ville / Adresse') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Prix') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Livreur') }}</th>
                            <th class="py-5 px-6 text-start">{{ __('Statut') }}</th>
                                ...
                                <td class="py-5 px-6">
                                    ...
                                    @else
                                        <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-amber-50 text-amber-700 border border-amber-100/60 flex items-center gap-1.5 w-fit">
                                            <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span> {{ __('Non affecté') }}
                                        </span>
                                    @endif
                                </td>
                                   <td class="py-5 px-6">
                                ...
                                <td class="py-5 px-6 text-center">
                                    <div class="flex justify-center items-center gap-3.5">
                                        <a href="{{ route('admin.colis.edit', $colis->id) }}" class="text-slate-400 hover:text-[#0A4BB3] transition text-base">
                                            <i class="fas fa-edit" title="{{ __('Modifier') }}"></i>
                                        </a>

                                        <form action="{{ route('admin.colis.destroy', $colis->id) }}" method="POST" onsubmit="return confirm(@json(__('Êtes-vous sûr de vouloir supprimer définitivement ce colis ?')));" class="inline m-0 p-0">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="p-10 text-center text-slate-400 font-bold">
                                    <i class="fas fa-box-open text-2xl block mb-2 text-slate-300"></i> {{ __('Aucun colis trouvé dans la base de données.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
