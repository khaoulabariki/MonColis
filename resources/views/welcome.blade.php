...
<head>
    ...
    <title>Shipily - {{ __('La Livraison E-commerce Intelligente') }}</title>
    <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;800;900&display=swap" rel="stylesheet">
    <style>
        @if($isRtl) body { font-family: 'Tajawal', sans-serif; } @endif
...
<body class="bg-slate-50 font-sans antialiased text-slate-800">
        ...
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 bg-brand-blue rounded-xl flex items-center justify-center relative overflow-hidden shadow-md shadow-blue-900/10">
                    <div class="absolute inset-0 border-e-4 border-white/20 transform rotate-45 scale-150"></div>
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-[9px] font-bold text-slate-400 tracking-widest uppercase mt-1">{{ __('LIVRAISON SIMPLIFIÉE') }}</span>
                </div>
            </div>

            <div class="hidden md:flex items-center gap-8 text-sm font-semibold text-slate-600">
                <a href="#suivi" class="hover:text-brand-blue transition">{{ __('Suivre un Colis') }}</a>
                <a href="#services" class="hover:text-brand-blue transition">{{ __('Nos Services') }}</a>
                <a href="#workflow" class="hover:text-brand-blue transition">{{ __('Notre Processus') }}</a>
            </div>
            ...
            <div class="mt-12 max-w-2xl mx-auto">
                <form action="/tracking" method="GET" class="bg-white p-3 rounded-2xl shadow-2xl border border-slate-100/80 flex flex-col md:flex-row gap-2.5">
                    <div class="flex-1 flex items-center gap-3 px-3">
                        ...
                        <input type="text" name="code" value="{{ $code ?? '' }}" required placeholder="{{ __('Entrez votre code de suivi') }}"
                    ...
                    <button type="submit" class="bg-brand-blue hover:bg-brand-blue-dark text-white font-black px-8 h-12 rounded-xl transition shadow-lg shadow-blue-200 whitespace-nowrap cursor-pointer flex items-center justify-center gap-2 text-sm">
                        <i class="fas fa-crosshairs"></i> {{ __('Suivre votre Colis') }}
                    </button>
                </form>
            </div>

            @if($colis)
            <div class="mt-12 bg-white rounded-3xl shadow-xl border border-slate-100 p-8 max-w-2xl mx-auto text-start transition-all duration-300">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-6 mb-8">
                    <div>
                        <span class="text-[10px] font-black uppercase text-slate-400 tracking-widest">{{ __('Référence Unique') }}</span>
                ...
                <div class="relative flex flex-col justify-between h-48 ps-8 before:absolute before:start-[11px] before:top-2 before:bottom-2 before:w-[3px] before:bg-slate-100 mb-8">

                    <div class="absolute start-[11px] top-2 w-[3px] bg-brand-blue transition-all duration-500"
                         style="height: {{ $isFinal ? '100%' : ($isEnCours ? '50%' : ($isRamasse ? '0%' : '0%')) }}">
                    </div>

                    <div class="relative flex items-center gap-4">
                        <div class="absolute -start-[31px] w-6 h-6 rounded-full border-4 {{ $isRamasse ? 'bg-brand-blue border-blue-100 z-10' : 'bg-white border-slate-200' }} flex items-center justify-center transition-all">
                            @if($isRamasse) <i class="fas fa-check text-[9px] text-white"></i> @endif
                        </div>
                        <h4 class="text-sm font-black {{ $isRamasse ? 'text-slate-900' : 'text-slate-400' }}">{{ __('Ramassé') }}</h4>
                    </div>

                    <div class="relative flex items-center gap-4">
                        <div class="absolute -start-[31px] w-6 h-6 rounded-full border-4 {{ $isEnCours ? 'bg-brand-blue border-blue-100 z-10' : 'bg-white border-slate-200' }} flex items-center justify-center transition-all">
                            @if($isEnCours) <i class="fas fa-truck text-[9px] text-white rtl:-scale-x-100"></i> @endif
                        </div>
                        <h4 class="text-sm font-black {{ $isEnCours ? 'text-slate-900' : 'text-slate-400' }}">{{ __('En cours') }}</h4>
                    </div>

                    @if($isChorfaNormal)
                        <div class="relative flex items-center gap-4">
                            <div class="absolute -start-[31px] w-6 h-6 rounded-full border-4 {{ $currentStatut == 'livré' || $currentStatut == 'livre' ? 'bg-emerald-500 border-emerald-100 z-10' : 'bg-white border-slate-200' }} flex items-center justify-center transition-all">
                                @if($currentStatut == 'livré' || $currentStatut == 'livre') <i class="fas fa-check text-[9px] text-white"></i> @endif
                            </div>
                            <h4 class="text-sm font-black {{ $currentStatut == 'livré' || $currentStatut == 'livre' ? 'text-emerald-600' : 'text-slate-400' }}">{{ __('Livré') }}</h4>
                        </div>
                    @else
                        <div class="relative flex items-center gap-4">
                            <div class="absolute -start-[31px] w-6 h-6 rounded-full border-4 bg-rose-500 border-rose-100 z-10 flex items-center justify-center">
                                <i class="fas fa-times text-[9px] text-white"></i>
                            </div>
                            <h4 class="text-sm font-black text-rose-600">
                                {{ in_array($currentStatut, ['annulé', 'annule']) ? __('Annulé') : __('Retourné') }}
                            </h4>
                        </div>
                    @endif
                </div>
                ...
                @if(in_array($currentStatut, ['livré', 'livre']))
                    <div class="border-t border-slate-100 pt-6 mt-6">
                        <h3 class="text-xs font-black text-slate-800 uppercase tracking-wider mb-3 flex items-center gap-2">
                            <i class="fas fa-comment-alt text-brand-blue"></i> {{ __('Laissez votre avis sur la livraison') }}
                        </h3>
                        ...
                        <form action="{{ route('avis.store', $colis->token_suivi) }}" method="POST" class="space-y-3">
                            ...
                            <textarea name="commentaire" rows="3" required
                                placeholder="{{ __('Votre avis nous intéresse') }}"
    ...
    <section id="services" class="py-20 bg-brand-blue relative">
        <div class="max-w-6xl mx-auto px-6 relative z-10">
            <div class="text-center mb-16">
                <h3 class="text-3xl font-black text-white">{{ __('Les apports de la plateforme Shipily') }}</h3>
                <p class="text-blue-200 text-sm mt-2 max-w-xl mx-auto font-medium">{{ __("L'écosystème logistique moderne et intelligent pensé exclusivement pour les e-commerçants marocains.") }}</p>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="p-8 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10">
                    <div class="w-12 h-12 bg-brand-orange text-white rounded-xl flex items-center justify-center text-xl mb-6 shadow-lg">
                        <i class="fas fa-shipping-fast rtl:-scale-x-100"></i>
                    </div>
                    <h4 class="text-lg font-bold text-white mb-3">{{ __('Livraison Express Nationale') }}</h4>
                    <p class="text-xs text-blue-100 leading-relaxed font-medium">{{ __('Nous expédions et distribuons vos colis partout au Maroc sous un délai garanti de 24h à 48h maximum.') }}</p>
                </div>
                <div class="p-8 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10">
                    <div class="w-12 h-12 bg-emerald-500 text-white rounded-xl flex items-center justify-center text-xl mb-6 shadow-lg">
                        <i class="fas fa-wallet"></i>
                    </div>
                    <h4 class="text-lg font-bold text-white mb-3">{{ __('Gestion Flexible des Fonds') }}</h4>
                    <p class="text-xs text-blue-100 leading-relaxed font-medium">{{ __('Vos encaissements de livraison (Cash on Delivery) sont traités et versés de manière fluide et transparente sur votre compte.') }}</p>
                </div>
                <div class="p-8 bg-white rounded-2xl border border-white shadow-xl">
                    ...
                    <h4 class="text-lg font-bold text-slate-900 mb-3">{{ __('Suivi Prédictif par IA') }}</h4>
                    <p class="text-xs text-slate-500 leading-relaxed font-medium">{{ __("Un moteur d'intelligence artificielle intégré analyse le sentiment des avis clients pour anticiper et résoudre automatiquement les anomalies de livraison.") }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="workflow" class="py-20 bg-white">
        <div class="max-w-6xl mx-auto px-6">
            <div class="text-center mb-16">
                <span class="text-brand-blue text-xs font-black uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-md">{{ __('Flux de Travail') }}</span>
                <h3 class="text-3xl font-black text-slate-900 mt-2">{{ __('Comment fonctionne Shipily ?') }}</h3>
                <p class="text-slate-400 text-sm mt-2">{{ __('Un workflow automatisé et transparent de A à Z.') }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
                <div class="text-center relative">
                    ...
                    <h4 class="text-base font-bold text-slate-900 mb-1">{{ __('Préparation & Dépôt') }}</h4>
                    <p class="text-xs text-slate-400 max-w-[200px] mx-auto">{{ __("L'E-commerçant crée le colis sur son tableau de bord et génère le code unique.") }}</p>
                </div>
                ...
                <div class="text-center relative">
                    ...
                    <h4 class="text-base font-bold text-slate-900 mb-1">{{ __('Affectation Auto') }}</h4>
                    <p class="text-xs text-slate-400 max-w-[200px] mx-auto">{{ __("L'administrateur attribue intelligemment le colis au livreur de la zone concernée.") }}</p>
                </div>
                ...
                <div class="text-center relative">
                    ...
                    <h4 class="text-base font-bold text-slate-900 mb-1">{{ __('Livraison Réelle') }}</h4>
                    <p class="text-xs text-slate-400 max-w-[200px] mx-auto">{{ __('Le livreur remet le colis, encaisse le montant (COD) et change le statut en temps réel.') }}</p>
                </div>
                ...
                <div class="text-center relative">
                    ...
                    <h4 class="text-base font-bold text-slate-900 mb-1">{{ __('Wallet & Analyse IA') }}</h4>
                    <p class="text-xs text-slate-400 max-w-[200px] mx-auto">{{ __("Les fonds sont versés instantanément sur le portefeuille, et l'IA analyse l'avis du client.") }}</p>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 text-sm">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-brand-blue rounded-lg flex items-center justify-center relative overflow-hidden shadow-xs">
                    <div class="absolute inset-0 border-e-2 border-white/20 transform rotate-45 scale-150"></div>
            ...
            <p class="text-xs font-bold text-slate-400">© 2026 Shipily. {{ __('Tous droits réservés.') }} <span class="text-brand-blue ms-1">#SmartLogistics</span></p>
        </div>
    </footer>
