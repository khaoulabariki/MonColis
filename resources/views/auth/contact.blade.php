...
<head>
    ...
    <title>{{ __('Contactez-nous') }} - Shipily</title>
...
<body class="bg-slate-50 font-sans antialiased text-slate-800">
        ...
        <div class="max-w-7xl mx-auto px-6 w-full flex items-center justify-between">
            <a href="/tracking" class="flex items-center gap-3">
                <div class="w-10 h-10 bg-brand-blue rounded-xl flex items-center justify-center relative overflow-hidden">
                    <i class="fas fa-arrow-left text-white text-sm rtl:rotate-180"></i>
                </div>
                <span class="text-sm font-bold text-slate-500 hover:text-brand-blue transition">{{ __("Retour à l'accueil") }}</span>
            </a>
    ...
    <main class="py-16 max-w-5xl mx-auto px-6">
        <div class="text-center mb-12">
            <span class="text-brand-orange text-xs font-black uppercase tracking-widest bg-orange-50 px-3 py-1 rounded-md">{{ __('Support Client') }}</span>
            <h1 class="text-3xl md:text-4xl font-black text-slate-900 mt-2">{{ __("Contacter l'administration") }}</h1>
            <p class="text-slate-400 text-sm mt-2 max-w-md mx-auto">{{ __('Une question sur vos colis ou vos paiements ? Notre équipe vous répond dans les plus brefs délais.') }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="md:col-span-1 space-y-4">
                <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <h5 class="text-xs text-slate-400 font-bold uppercase">{{ __('Téléphone') }}</h5>
                        <p class="text-sm font-bold text-slate-800" dir="ltr">+212 5XX XX XX XX</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <h5 class="text-xs text-slate-400 font-bold uppercase">{{ __('Email Support') }}</h5>
                ...
                <a href="https://wa.me/21260000000" target="_blank" class="block bg-emerald-500 hover:bg-emerald-600 text-white p-5 rounded-2xl shadow-md transition text-center font-bold text-sm flex items-center justify-center gap-2">
                    <i class="fab fa-whatsapp text-xl"></i> {{ __('Support WhatsApp Direct') }}
                </a>
            </div>
            ...
            <div class="md:col-span-2 bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
                ...
                <form action="{{ route('contact.post') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase">{{ __('Nom Complet') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase">{{ __('Email') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase">{{ __('Sujet de votre réclamation') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase">{{ __('Message (Détails du problème)') }}</label>
                    ...
                    <button type="submit" class="bg-brand-blue hover:bg-brand-blue-dark text-white font-bold text-sm px-6 py-3 rounded-xl transition shadow-md w-full sm:w-auto cursor-pointer">
                        {{ __("Envoyer le message à l'administration") }}
                    </button>
                </form>
            </div>
        </div>
