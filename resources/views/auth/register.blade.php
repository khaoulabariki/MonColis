...
<head>
    ...
    <title>Shipily - {{ __('Inscription') }}</title>
...
<body class="bg-[#F8FAFC] antialiased text-slate-800 min-h-screen flex flex-col justify-between">
    ...
    <main class="flex-1 flex flex-col items-center justify-center px-6 pb-24">
        ...
        <div class="w-full max-w-sm">
            ...
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight mb-1">{{ __('Créer un compte') }}</h1>
            <p class="text-slate-400 text-sm mb-6">{{ __('Rejoignez Shipily et créez votre espace professionnel.') }}</p>
            ...
            <form action="{{ route('register.post') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label for="role" class="block text-sm font-semibold text-slate-700 mb-1.5">{{ __('Vous êtes ?') }}</label>
                    <select name="role" id="role" required
                        ...
                        <option value="" disabled selected>{{ __('Choisir votre profil...') }}</option>
                        <option value="ecommercant">🛍️ {{ __('E-commerçant (Vendeur)') }}</option>
                        <option value="livreur">🚚 {{ __('Livreur (Livreur indépendant)') }}</option>
                    </select>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">{{ __('Nom') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">{{ __('Prénom') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">{{ __('N° Téléphone') }}</label>
                ...
                <button type="submit" class="w-full bg-brand-blue hover:opacity-95 text-white h-12 rounded-full font-bold text-sm transition shadow-md shadow-blue-900/10 flex items-center justify-center gap-2 cursor-pointer mt-3">
                    {{ __('Créer mon compte professionnel') }} <i class="fas fa-arrow-right text-xs rtl:rotate-180"></i>
                </button>
            </form>
            ...
            <div class="mt-8 text-center text-sm">
                <p class="text-slate-400 font-medium">
                    ...
                    <a href="{{ route('login') }}" class="text-brand-orange font-bold hover:underline ms-0.5">{{ __('Connectez-vous') }}</a>
                </p>
            </div>
        </div>
    </main>
